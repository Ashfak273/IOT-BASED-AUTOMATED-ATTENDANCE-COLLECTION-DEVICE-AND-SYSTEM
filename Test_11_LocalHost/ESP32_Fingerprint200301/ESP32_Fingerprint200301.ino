...
//Fingerprint-----------------------------
#include <SPI.h>                 //for Software serial
...
const char *ssid = "AK";
const char *password = "mzashfak.273";
const char* device_token  = "cd89bdc3";


String getData, Link;
//used in checktoaddID
String URL = "http://192.168.8.101/biometricattendancev2/getdata.php"; //computer IP or the server domain



int FingerID = 0;    // The Fingerprint ID from the scanner
int t1, t2;          // below using in setup func for check delete or add id                      
...
void connectToWiFi()
{
  ...
  uint32_t periodToConnect = 30000L; // saving value in variable
  for(uint32_t StartToConnect = millis(); (millis()-StartToConnect) < periodToConnect;) // returns the milliseconds
  {
    if ( WiFi.status() != WL_CONNECTED )
    {
      delay(500);
